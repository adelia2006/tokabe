...
$portfolios = App\Models\Portofolio::where('kategori', 10)->get();
foreach($portfolios as $p) {
    if (strpos($p->judul, 'Lanal TBA') !== false) {
    ...
    } elseif (strpos($p->judul, 'Simpati Nonton') !== false) {
        $p->judul = json_encode(['id' => 'Simpati Nonton Bareng', 'en' => 'Simpati Nonton Bareng']);
        $p->deskripsi = json_encode([
    ...
    } elseif (strpos($p->judul, 'Honda DBL') !== false) {
        $p->judul = json_encode(['id' => 'Honda DBL North Sumatera Series', 'en' => 'Honda DBL North Sumatera Series']);
        $p->deskripsi = json_encode([
